(function($){
  ...
  $.fn.combo = function(options){
    ...
    var config = $.extend({}, {
      ...
      template: {
        ...
        menu: '<ul class="ui-pixels-combo dropdown-menu"></ul>',
  ...
  var combo = function(container, config){
    ...
    $t.container.wrap('<div class="input-append ui-pixels-combo"></div>')
    .attr($t.config.attr || {})
    ...
    if(typeof($t.config.items) === 'string' || $t.config.length === 0){
      // parse data from data-items attribute
  ...
  $.extend(combo.prototype, {
    ...
    listen: function(){
      ...
      $(document.body).on('click', function(e){
        ...
        if($t.menu.is(':visible') && !target.is('.ui-pixels-combo') && !target.parents().is('.ui-pixels-combo'))
          $t.hide();
      });
    ...
    move: function(direction){
      ...
      if(!next){
        next = $t.menu.find('li')[up ? 'last' : 'first']();
      }
